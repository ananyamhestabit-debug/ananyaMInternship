...
#AutoGen ka output complex hota hai, Ye function final answer nikalta hai
def last_reply(chat_result) -> str:
    #agar summary hai → use karo
    if hasattr(chat_result, "summary") and chat_result.summary:
        return chat_result.summary.strip()
    #pura conversation
    history = chat_result.chat_history
    if len(history) >= 2:
        return history[1]["content"].strip()  #second message (agent reply)
...
#decide karta hai:kaunsa agent use hoga
def detect_intent(query: str) -> str:
    ...
    if any(k in q for k in FILE_KEYWORDS):
        return "FILE_ONLY"
    return _llm_detect_intent(query)  #fallback LLM classifier


# ek chhota agent banaya gaya, LLM se classification karwa rahe
...
""".strip(),
    ...
    if label not in ("CODE_ONLY", "DATA", "FILE_ONLY", "DB_ONLY"):
        return "CODE_ONLY"   
    return label  #final intent


#agar CSV nahi hai → create kar do
def seed_sales_csv():
    ...
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["product", "region", "units", "revenue"])  #headers
        w.writerows([
            ["Widget A", "North",  120, 14400],  #default data insert
...
def _run_code_only(query: str) -> str:
    ...
    cp = make_code_proxy(query=query)  #proxy create
    r = cp.initiate_chat(code_agent, message=query, max_turns=8, silent=False)  #agent+proxy communication
    return last_reply(r)
...
def _run_file_only(query: str) -> str:
    ...
    # Execute tool FIRST(LLM se pehle)
    tool_result = fp._dispatch(query.lower())

    # Send real data to LLM for summarization
    r = fp.initiate_chat(
        file_agent,
        #LLM ko data diya
        message=(
...
#SQL agent run: LLM SQL banata → proxy run karta
...
"""
    ...
    # SYNTHESIS :final answer combine
