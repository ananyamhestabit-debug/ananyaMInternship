...
def generate_embeddings(chunks_json_path, save_path):
    ...
    np.save(save_path, embeddings)  # .npy(numpy binary file) file saved 
    print(f"Saved {len(embeddings)} embeddings to {save_path}")
    return embeddings

if __name__ == "__main__":
    generate_embeddings("../data/chunks/chunks.json", "../data/embeddings/chunks.npy")  #chunks.json → embedder → chunks.npy