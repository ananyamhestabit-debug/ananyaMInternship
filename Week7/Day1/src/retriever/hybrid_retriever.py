...
#semantic+keyword: smart search engine -> for best chunks
class HybridRetriever:
    def __init__(self, chunks, embedding_model="all-MiniLM-L6-v2"):
        self.chunks = chunks
        self.model = SentenceTransformer(embedding_model)
        self.chunk_texts = [c["content"] for c in chunks]  #text list
        self.chunk_ids = [c["chunk_id"] for c in chunks]
        self.metadata = [c.get("metadata", {}) for c in chunks]

        # Precompute embeddings(speed)
        self.embeddings = self.model.encode(self.chunk_texts, convert_to_tensor=True)

        # Initialize BM25 for keyword fallback: query words -> document me kitni baar aye -> score
        tokenized = [text.split() for text in self.chunk_texts]
        self.bm25 = BM25Okapi(tokenized)  #keyword ranking algo

    def mmr(self, query_vec, top_k=5, lambda_param=0.7):  #retrieval function
        """
